...
class Cart {
  ...
  constructor() {
    ...
    this.modalAddButton = document.querySelector('[data-cart-modal-add]') || null;
  ...
  _events = () => {
    if (this.modalAddButton) this.modalAddButton.addEventListener('click', this._handlerAddCartFromModal)
  }
    ...
    ar1.forEach(item => {
      ar2.forEach(item2 => {
        ...
        if (item.name === item2.name
            && item.inProduct !== item2.inProduct
        ) {
          status = 0;
        }
      })
  ...
  */
  _equalInCart = (idOrProduct, callback) => {
    let product = typeof idOrProduct === 'string' ? this.products.find(p => p.id === idOrProduct) : idOrProduct;
    ...
    cards.forEach(card => {
      if (card.element.ingredients) {
        if (!!this._equalIngredients(card.element.ingredients, product.ingredients)) {
  ...
  _resultPrice = (product) => {
    ...
    if (product.ingredients)  {
      product.ingredients.forEach(ing => {
        if (ing.inProduct) {price = price + product.count * ing.price}
      })
    }
  ...
  _initDopSlider = () => {
    return new Swiper ('.cart__dop-slider.swiper-container', {
      navigation: {
        nextEl: '.cart__dop-navs .cart__dop-nav_next',
        prevEl: '.cart__dop-navs .cart__dop-nav_prev',
      },
      slidesPerView: 'auto',
      spaceBetween: 12,
      freeMode: true,
  ...
  _renderProductCart = (id) => {
    ...
    if (product.cartType === 'ellipse') card.classList.add('product-card_ellipse');
    ...
    button.classList.add('button', 'button_orange', 'button_small', 'product-card__button');
    button.type = 'button';
    button.dataset.cartProduct = product.id;
    button.innerHTML = `<img class="icon first" src="./assets/img/i-cart-add.svg"> В корзину`;
    button.addEventListener('click', this._handlerOpenModal.bind(this, product.id));

    imgWrap.append(imgProduct)
    if (product.cartType !== 'ellipse') imgWrap.append(imgVapor)
  ...
  //создание листа продуктов на главной странице
  _renderMainList = () => {
    const listElms = document.querySelectorAll('[data-products-list]');
    if (!listElms) return;
    [].forEach.call(
      listElms,
      wrap => {
        const list = wrap.getAttribute('data-products-list').split(',')

        list.forEach(el => {
          const card = this._renderProductCart(el);
          if (!card) return;
  ...
  _renderIngredients = (product) => {
    ...
    wrap.className = `f_xs cart-item__ingredients ingredients${product.ingShow ? ' visible' : ''}`;
    wrap.dataset.ingredients;

    const title = document.createElement('div');
    title.className = 'ingredients__title';
    title.dataset.ingredientsHead = '';
    title.innerHTML = 'Изменить ингридиенты <img class="icon" src="./assets/img/i-top.svg">'
    ...
    product.ingredients.forEach(item => {
      ...
      label.append(input)
      label.append(span)
      label.append(`${item.name} ${item.price ? '+' + item.price + ' руб' : ''}`)
  ...
  _rednderDopCard = (id) => {
    ...
    const card = document.createElement('div');
    card.className = 'swiper-slide cart-item-dop';
    card.addEventListener('click', () => {
      this._handlerAddToCard(product.id)
    })
  ...
  _renderDopSlider = () => {
    const listElms = document.querySelectorAll('[data-dop-products-list]');
    if (!listElms) return;

    [].forEach.call(
      listElms,
      wrap => {
        const list = wrap.getAttribute('data-dop-products-list').split(',')

        list.forEach(el => {
          const card = this._rednderDopCard(el);
          if (!card) return;
  ...
  _toastDefault = () => {
    toast.default({
      content: '<div class="msg"><img class="icon first" src="./assets/img/i-ok.svg" />Товар добавлен в корзину</div>'
    })
  }
}
...
class Toast {
  ...
  createNode = (name, attr, append, content) => {
      ...
      for(var v in attr) { if (attr.hasOwnProperty(v)) node.setAttribute(v, attr[v]) }
  ...
  render = (options = {}) => {
    ...
    if (options.hasOwnProperty('header')) {
        var toastHeader = this.createNode('div', {
            "class": 'toast__header'
        }, toastItem, options.header)
    }

    if (options.hasOwnProperty('content')) {
        var toastBody = this.createNode('div', {
            "class": 'toast__body'
        }, toastItem, options.content)
    }

    if (options.hasOwnProperty('line') && options.line === true) {
        var toastLine = this.createNode('span', {
            "class": 'toast__line',
    ...
    if (options.hasOwnProperty('closeBtn') && options.closeBtn === true) {
      var toastClose = this.createNode('button', {
        "class": 'toast__close',
        "type": "button",
        "aria-label": "Скрыть"
      }, toastItem, '&times;')

      toastClose.addEventListener("click", () => {this.remove(toastItem, options); });
    }
